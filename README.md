# TRIPTURE 
capture your trip, Tripture


## ~~[Tripture 다운로드](https://onesto.re/0000778618)~~

## 1. 프로젝트 소개 🗒
<img width="960" alt="projectImg" src="https://github.com/user-attachments/assets/1855541e-0eaa-4d91-8190-a7cf504763e7">

**_여행 경비 절약과 정보 공유를 위한 트레블 앱테크 서비스_**


## 2. 프로젝트 기간
- 기획 : _2024-04-28 ~ 2024-08-05_
- 개발 : _2024-06-01 ~ 2024-09-25_

## 3. 팀원 소개
FE - 신정아, 장다연, 최지민
| BE - 장다연, 최지민

## 4. 프로젝트 흐름도 📌
### 관광지 검색 기능 흐름도

|<img width="200" alt="projectImg" src="https://github.com/user-attachments/assets/f86863b7-94b4-4f42-873a-dfac809bab59">|<img width="200" alt="projectImg" src="https://github.com/user-attachments/assets/98e93d55-3b95-4375-a70f-2d75e5760757">|<img width="200" alt="projectImg" src="https://github.com/user-attachments/assets/2445eeff-e734-4f26-a770-6a6edf685801">|<img width="200" alt="projectImg" src="https://github.com/user-attachments/assets/49414dad-2d49-4240-81b1-1ca9460d5584">|
|---|---|---|---|
|8도로 구분된 아이콘 중<br>여행하고 싶은 지역의 아이콘을 클릭한다.|지역 필터링이 된 관광지 목록 화면으로 이동한다.|선택한 관심 있는 관광지의 상세 정보를 보여준다.|지도보기를 클릭해 관광지의 위치를 파악하고, 주변 관광지를 확인한다.|

<br>

### 포토챌린지 참여 기능 흐름도

|<img width="200" alt="projectImg" src="https://github.com/user-attachments/assets/5e4618b4-e38a-4a30-a14a-a58452e59f37">|<img width="200" alt="projectImg" src="https://github.com/user-attachments/assets/ea8e9afe-19c8-4c0e-9627-ee3a0bfa4659">|<img width="200" alt="projectImg" src="https://github.com/user-attachments/assets/71566173-2ac5-46c5-a042-37038d2ff616">|<img width="200" alt="projectImg" src="https://github.com/user-attachments/assets/2c3a5c04-e02d-47c0-b33c-0c59fed942a4">|
|---|---|---|---|
|포토챌린지 탭을 클릭해 다른 사람들이 올린 게시물을 확인한다.|참여할 챌린지 혹은 관심있는 챌린지 게시물을 확인해 내용을 확인한다.|포토챌린지 참여하기 버튼을 눌러 사진, 내용을 등록해 리워드를 획득한다.|등록된 게시물은 커뮤니티에 업로드 된다.<br>커뮤니티에 업로드된 글은 다른 사용자들이 확인할 수 있고, 관광지 정보 또한 확인할 수 있다.|

1번 사진에서, 일반 챌린지의 경우 챌린지 참여 버튼 클릭 시 세 번째 사진으로 이동한다.
- 히든 챌린지의 경우 해당 흐름도대로 이동한다.
- 챌린지 참여 버튼은 위 1번 흐름도에서 3, 4번 사진의 참여하기 버튼이다.

<br>

### 마이페이지 내 챌린지 /포인트/스크랩 현황 확인 기능 흐름도

|<img width="200" alt="projectImg" src="https://github.com/user-attachments/assets/24b230c3-f6d7-49cf-8cb1-ecbfe8f1ab62">|<img width="200" alt="projectImg" src="https://github.com/user-attachments/assets/dd757cb1-675d-4aa7-aa73-387a00dd7b6c">|<img width="200" alt="projectImg" src="https://github.com/user-attachments/assets/f62e6803-81d2-4d28-9ed5-27a8c1f111db">|<img width="200" alt="projectImg" src="https://github.com/user-attachments/assets/c990b294-2ef8-4058-b323-092edc55309b">|
|---|---|---|---|
|마이페이지 탭에서 자신이 작성한 게시글, 챌린지 레벨을 확인한다.|챌린지 현황에서 지역별로 자신이 수행한 챌린지 개수, 레벨을 확인한다.|포인트 탭에서 내가 모은 포인트, 포인트 사용 내역을 확인한다.|스크랩 탭에서 내가 저장한 관광지, 내가 저장한 챌린지를 확인한다.|

<br>

### 포인트 상점 구매 기능 흐름도

|<img width="200" alt="projectImg" src="https://github.com/user-attachments/assets/5373e4b7-2fef-4ffa-8916-a7f79d5a2a70">|<img width="200" alt="projectImg" src="https://github.com/user-attachments/assets/d32e4a2a-733c-4721-8beb-20d116464c2c">|<img width="200" alt="projectImg" src="https://github.com/user-attachments/assets/93969700-ed51-46b6-a908-4e74abc35901">|<img width="200" alt="projectImg" src="https://github.com/user-attachments/assets/01b69a0c-948e-4094-a073-db0700a19d85">|
|---|---|---|---|
|포인트 상점 탭에서 등록된 여행 상품(티켓) 목록을 확인한다.|구매하고 싶은 상품을 클릭해 상품의 상세 정보를 확인한다.|상품의 가격, 수량을 확인한다.<br>만약 포인트가 부족하다면, 결제창으로 넘어가지 못한다.|자신이 보유한  포인트를 사용해 상품을 구매한다.|

## 7. 기술 스택 🛠
<img width="960" alt="SA" src="https://github.com/user-attachments/assets/a22ad4cc-39d3-4ef7-afe0-7459e71ba565">

## 8. WORKFLOW 🫧

## 9. API 명세서 📃
https://documenter.getpostman.com/view/30136265/2sA3kYifMY

## 10. ERD 🗂

## 11. UI

## 12. 트러블 슈팅 🎃
